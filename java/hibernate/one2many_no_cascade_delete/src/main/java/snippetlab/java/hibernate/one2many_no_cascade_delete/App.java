package snippetlab.java.hibernate.one2many_no_cascade_delete;
...
public class App
{
		...
		main( String[] args )
	{
		...
		try {
			...
			theInstructorId = instructor.getId();

			System.out.println("=================== Before insert courses");
			System.out.println("=================== The instructor id: " + theInstructorId);
			...
			instructor = session.get(Instructor.class, theInstructorId);
			...
			// delete the course from DB and don't delete instructor
			session = factory.getCurrentSession();
			session.beginTransaction();

			System.out.println("=================== Ready to delete the course: " + tempCourse1);
